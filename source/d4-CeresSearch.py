...
def _get_direction_indexes_left_end(x: int, y: int, max_x: int, max_y: int, string_length: str) -> list[tuple[int, int]]:
    ...
    valid_direction_indexes = [
        direction for direction in all_direction_indexes 
        if all((step_x >=0 and step_y >=0) and (step_x<max_x and step_y<max_y) 
        for step_x, step_y in direction)
    ]
    ...
    for i in range(len(input)):
        for j in range(len(input[i])):
            if input[i][j] == 'X':
                ...
                candidate_xmas = [
                    ''.join([input[x][y] for x,y in candidate]) for candidate in direction_indexes
                ]
